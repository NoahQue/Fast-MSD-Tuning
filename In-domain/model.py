...
class GNN_graphpred(torch.nn.Module):
    ...
    #         key[len("gnn."):] if key.startswith("gnn.") else key: value
    #         for key, value in state_dict.items()
    #     }

    #     # 加载修正后的 state_dict
    #     self.gnn.load_state_dict(corrected_state_dict, strict=True)  # 设置 strict=True 保证加载完全匹配
    ...
    def from_pretrained(self, model_file):
        ...
        corrected_state_dict = {
            key[len("gnn."):] if key.startswith("gnn.") else key: value
            for key, value in state_dict.items()
            if key not in ["graph_pred_linear.weight", "graph_pred_linear.bias"]  # 忽略特定参数
        }

        # 加载修正后的 state_dict
        self.gnn.load_state_dict(corrected_state_dict, strict=True)  # 设置 strict=False 忽略其他不匹配键
