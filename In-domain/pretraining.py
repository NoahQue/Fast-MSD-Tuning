...
def main():
    ...
    parser.add_argument('--dataset', type=str, default = 'pcqm4m-v2', help='root directory of dataset for pretraining')
    ...
    # 初始化一个变量来存储最低的损失
    best_loss = float('inf')  # 初始值设置为无穷大
    best_model_path = None  # 用于保存最佳模型的文件路径
    ...
    for epoch in range(1, args.epochs+1):
        ...
        if train_loss < best_loss:
            best_loss = train_loss  # 更新最小损失
            best_model_path = output_file_temp + f"_best.pth"  # 更新保存路径
        ...
        if not resume and epoch % 5 == 0:
            checkpoint_path = output_file_temp + f"_{epoch}.pth"
    ...
    # 最后输出最低损失的模型文件路径
    if best_model_path:
        print(f"Lowest loss model saved as: {best_model_path}")
